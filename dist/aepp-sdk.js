...
var WebsocketProvider = require('./providers/ws');

var Crypto = require('./utils/crypto');

var HdWallet = require('./utils/hdwallet');
/**
...
function () {
   ...
   * @param provider
   */
  function AeternityClient(provider) {
    _classCallCheck(this, AeternityClient);

    this.provider = provider; // Semantically close services are bundeled in classes
  ...
  _createClass(AeternityClient, [{
    key: "addSubscription",
    value: function addSubscription(subscription) {
      if (this.provider.addSubscription && typeof this.provider.addSubscription === 'function') {
    ...
    value: function removeSubscription(subscription) {
      if (this.provider.removeSubscription && typeof this.provider.removeSubscription === 'function') {
    ...
    value: function addConnectionListener(listener) {
      if (this.provider.addConnectionListener && typeof this.provider.addConnectionListener === 'function') {
    ...
    value: function removeConnectionListener(listener) {
      if (this.provider.removeConnectionListener && typeof this.provider.removeConnectionListener === 'function') {
        this.provider.removeConnectionListener(listener);
      }
    }
  }]);

  return AeternityClient;
}();
...
AeternityClient.Crypto = Crypto;
AeternityClient.HdWallet = HdWallet;
